class TabLink {
  constructor(element){
    ...
    // If the data is 'all' then select all cards regardless of their data attribute
      ...
      // else this.card = just the selected category
    
      if(this.tabData === 'all'){
        this.cards = document.querySelectorAll('.card');
    } else {
      this.cards = document.querySelectorAll(`.card[data-tabs='${this.tabData}']`);
    }
    ...
    this.cards = Array.from(this.cards).map(tab => new TabCard(tab));

// vid example was etc.map(button => new Button(button));

    // Add a click event that invokes selectTab
    // when clicked, select the category
  ...
  // selectTab closing bracket
}
// TabLink closing bracket
class TabCard {
  constructor(element){
