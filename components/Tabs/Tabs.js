class TabLink{
  constructor(element) {
    ...
    this.cards = Array.from(this.cards).map(cardsItem => new TabCard(cardsItem));
  ...
  selectTab(){
    ...
    this.cards.forEach(element => element.selectCard());
  }
}
...
tabs = Array.from(tabs).map(tabItems => new TabLink(tabItems))
