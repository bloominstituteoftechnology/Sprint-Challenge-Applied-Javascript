class TabLink {
    constructor(tabElement) {
        ...
        console.log('Looking At:', tabElement);
        console.log('A type of: ', typeof tabElement);
        ...
        this.cards = Array.from(this.cards).map((create) => { return new TabCard(create) });

        // Add a click event that invokes this.selectTab
        this.tabElement.addEventListener('click', (e) => { return this.selectTab(e) });
    }
    ...
    selectTab() {
        ...
        tabs.forEach((one) => { return one.classList.remove('.active-tab') })

        // Select all of the elements with the .card class on them
        const cards = document.querySelectorAll('.card');

        // Iterate through the NodeList setting the display style each one to 'none'
        cards.forEach((one) => { return one.style.display = 'none' });
