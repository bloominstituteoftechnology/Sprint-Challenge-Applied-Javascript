...
class TabLink {
  constructor(element, parent){
    ...
    this.cards = this.cards.map((i, card) => 
      new TabCard(card));
  ...
  selectTab(){
    ...
    this.cards.each(function(){this.selectCard()})
    // this.cards.each((i, card) => card.selectCard()); // ES 6 syntax
  }
...
class Tabs {
  ...
  getCards(data){  //this data is the data-tab attr of the tab
    let cards = $('.card')
    ...
    if (data === 'all') {
      ...
      return cards.filter((i, card) => $(card).data('tab') === data)
    }
  }
}
