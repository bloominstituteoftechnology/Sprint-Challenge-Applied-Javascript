class TabLink {
  constructor(element){
    ...
    this.cards = Array.from(this.cards).map(function(card_el){
      return new TabCard(card_el);
    });
  ...
  selectTab(){
    ...
    tabs.forEach(function(tab_el){
      tab_el.classList.remove('active-tab');
    });
...
tabs = Array.from(tabs).map(function(tab_el){
  return new TabLink(tab_el)
});
