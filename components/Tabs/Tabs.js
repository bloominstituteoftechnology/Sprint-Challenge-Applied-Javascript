class Tabs {
  ...
  getCards(data){
    ...
    console.log(data, "this is input");
    if(data === "all") {
...
class TabLink {
  constructor(element, parent){
    ...
    this.element = element;
    console.log("beginning of TabLink")
    console.log(this.element);
    // assign this.parent to the parent reference
    this.parent = parent;
    console.log(parent);
    console.log("end of Tablink");
    ...
    this.cards = Array.from(this.cards).map(cardRef => new TabCard(cardRef));
...
tabs = Array.from(tabs).map(tabRef => new Tabs(tabRef));
