class TabCard {
  ...
  selectCard() {
    // show the card
    this.element.addClass('topics');
  }
  deselectCard() {
    // hide the card
    this.element.removeClass('topics');
  }
}
...
class TabLink {
  constructor(element, parent) {
    ...
    this.cards = this.cards.map((index, item) => new TabCard(card));
  ...
  selectTab(){
    ...
    this.cards.each((index, item) => card.selectCard());
  }
...
class Tabs {
  ...
  getCards(data) {
    ...
    return theCards.filter((index, card) => $(card).data('tab') === data);
  }
}
