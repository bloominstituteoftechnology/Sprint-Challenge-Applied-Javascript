class Tabs {
  ...
  updateActive(tabElement){
    ...
    // assign activeTab to tabElement to update it's appearance
    this.activeTab = tabElement;
  }

  getCards(data){
    // Update the logic in the if statement to check if 'all' is equal to the passed in data.
    if( data === 'all') {
...
class TabLink {
  constructor(element, parent){
    ...
    this.cards = Array.from(this.cards).map(function createNewTabCard (tabLink) {
      return new TabCard(tabLink);
    });
...
tabs = Array.from(tabs).map(function createTab (tab) {
  return new Tabs(tab);
});
