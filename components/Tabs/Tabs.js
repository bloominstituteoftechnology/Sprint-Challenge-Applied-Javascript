class TabLink {
  constructor(element){
    ...
    this.cards = document.querySelector(`.card[data-tab="${this.tab}"`);
    ...
    this.cards = Array.from(this.cards).map((element,index) => {return new TabCard(element)});
...
tabs = Array.from(tabs).map((element,index) => {return new TabLink(element)});
