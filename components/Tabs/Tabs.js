class Tabs {
  constructor(element){
    ...
    this.tabLang = document.querySelectorAll('.tab');
    // Notice that we are passing a new tab AND a reference to Tabs by using "this"
    this.tabLangArr = Array.from(this.tabLang).map( tabLan => new TabLink(tabLan, this) );
    // Assign activeTab to the first item in the tabs array
    this.activeTab = this.tabLangArr[0];
...
class TabLink {
  constructor(element, parent){
    ...
    this.newsCards = this.parent.getCards(this.element.dataset.tab);
    ...
    this.newsCards = Array.from(this.newsCards).map(cardObj => new TabCard(cardObj));
  ...
  selectTab(){
    ...
    this.newsCards.forEach(card => card.selectCard());
  }
  ...
  deselectTab(){
    ...
    this.newsCards.forEach( card => card.deselectCard());
  }
}
...
// //Old Code that I am not sure about and lost in
...
//     // not sure if this is the answer: this.tabs = Array.from(this.tabs).map( tab => {return new TabLink(tab, this) });
//     // console.log(this.tabs);
...
//   init(){
//     // Invoke the selectTab() method on activeTab so we can see the tab when the page loads.
//     // this.activeTab.selectTab(this.activeTab);
//   }
...
//     // assign activeTab to tabElement to update it's apperance
//     this.activeTab = tabElement.classList.toggle('.tab');
//   }
...
//       // Return a reference to the data attributes of all the ".card" classes.  Hint: use the passed data value in getCards() to accomplish this.
//       return document.querySelectorAll(`.card[data-tab:${this.card}`) ;
//     }
//   }
// }//Tabs
...
//     // Add a class of ".active-tab" to this.element
//     this.element.querySelector('.active-tab');
//     // Notice we are looping through the this.cards array and invoking selectCard() from the TabCard class, nothing to update here
