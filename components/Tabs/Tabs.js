...
class TabLink {
  constructor(element, parent){
    ...
    this.element = this.element.click(() => this.selectTab());
  }
  ...
  selectTab(){
    ...
    $.each(this.cards, (ind, val) => {
      if (this.element[0].dataset.tab === 'all') {
        val.selectCard();
      }
  ...
  deselectTab(){
    ...
    // this.cards;
    $.each(this.cards, (ind, val) => val.deselectCard());
  }
}
