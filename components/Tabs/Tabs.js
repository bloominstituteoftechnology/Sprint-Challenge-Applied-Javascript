class TabLink {
  constructor(element){
    ...
    this.tabData = element.getAttribute( "data-tab" ); 
    ...
    if( element.className === "card" && tabData ){
      this.cards = document.querySelectorAll( ".card" ) ;
    } else {
      this.cards = document.querySelectorAll( ".all" );
    }
    ...
    this.cards = Array.from(this.cards).map( (tabElm) =>{
      return new TabCard( tabElm );
    } );
  ...
  selectTab(){
    ...
    tabs.forEach( (tabElm) =>{
      return tabElm.classList.remove( ".active-tab" );
    });
    // Add a class of ".active-tab" to this.element
    this.element.setAttribute( ".active-tab" );
    ...
    cards.forEach( (cardElm) =>{
      return cardElm.style.display = "none";
    } )
...
tabs = Array.from(tabs).map( (tabElm) =>{
  return new TabLink( tabElm );
} )
