class TabLink {
  constructor(element){
    ...
    this.cards = Array.from(this.cards).map(tabCard => {return new TabCard(tabCard)});
  ...
  selectTab(){
    ...
    tabs.forEach(item => {return item.classList.remove('active-tab')});
    ...
    cards.forEach(item => {return item.style.display = 'none'})
...
tabs = Array.from(tabs).map( tab => {return new TabLink(tab)});
