class TabLink {
  constructor(element){
    ...
    if(this.element.dataset.tab === "all"){
      this.cards = document.querySelectorAll(`.tab active-tab`); //????****
    } else {
  ...
  selectTab(){
    ...
    const tabs = document.querySelectorAll('.tab'); //???????****repeat of first step?
    // Iterate through the NodeList removing the .active-tab class from each element
    tabs.forEach( tab => {
      tab.classList.remove('active-tab') //dot or no????***
    });
