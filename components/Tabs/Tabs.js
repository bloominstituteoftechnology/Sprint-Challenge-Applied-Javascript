...
class TabLink {
  constructor(element, parent){
    ...
    this.cards = this.cards.map((i,card) => new TacCard(card));
...
class Tabs {
  ...
  getCards(data){
    ...
    // Otherwise, only cards matching the data attribute should be return
    let allCards =$('.card');
  ...
  return allCards.filter((i,card) => $(card).data('tab'));
