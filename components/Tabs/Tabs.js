class Tabs {
  constructor(element){
    ...
    this.tabs = Array.from(this.tabs).map(link => {
       return new TabLink(link, this)
    });
    //-----------------problem is with Map function -----------------//
    console.log(`this.tabs2= ${this.tabs}`);
  ...
  getCards(data){
    ...
    if ('all' === data) {
      ...
      return this.element.querySelectorAll(`.card.dataset.tab="${data}"`);
    }
  }
}
