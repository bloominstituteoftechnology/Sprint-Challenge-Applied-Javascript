...
class TabLink {
  constructor(element, parent){
    ...
    this.cards = this.cards.map(function(index, card) {
      new TabCard(card, this);
    });
...
class Tabs {
  ...
  getCards(data){
    ...
    if($(`.card[data-tab ="${data}"]` === 'javascript')) {
      return $(`.card[data-tab ="${javascript}"]`);
    } else if ($(`.card[data-tab ="${data}"]` === 'technology')){
      return $(`.card[data-tab ="${technology}"]`);
    }else if ($(`.card[data-tab ="${data}"]` === 'node')){
      return $(`.card[data-tab ="${node}"]`);
    }else if ($(`.card[data-tab ="${data}"]` === 'jquery')){
      return $(`.card[data-tab ="${jquery}"]`);
    }else if ($(`.card[data-tab ="${data}"]` === 'bootstrap')){
      return $(`.card[data-tab ="${bootstrap}"]`);
    } else {
