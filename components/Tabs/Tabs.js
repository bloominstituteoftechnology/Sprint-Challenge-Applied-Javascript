class TabLink {
    constructor(element) {
        ...
        this.tabData = document.querySelector(`.tab[data-tab=${this.element.dataset.tab}]`).dataset.tab;

        // Find all elements with the .card class in index.html that correspond to the tab data attribute
        const allCardCLass = Array.from(document.querySelectorAll('.card'));

        // If the data is 'all' then select all cards regardless of their data attribute
        if (this.tabData === "all") {
            this.cards = allCardCLass;
        } else {
          this.cards = allCardCLass.filter(card => card.dataset.tab === this.tabData);
        }
