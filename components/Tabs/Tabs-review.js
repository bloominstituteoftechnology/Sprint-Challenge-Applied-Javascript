class Tabs {
  ...
  getCards(data){
    ...
    if ('all' === data) {
      ...
      return document.querySelectorAll('.card');//------changed to document
    } else {
      ...
      return document.querySelectorAll(`.card.dataset.tab="${data}"`);//------changed to document & this.element to .card
    }
  }// ------------- this is the filtering part
}

class TabLink {
  constructor(element, parent){ //tabs and parent is the same thing
    ...
    this.cards = this.parent.getCards(this.element.dataset.tab);//-----added this.parent on review
